...
import { LuCalendar as Calendar, LuChevronDown as ChevronDown, LuChevronLeft as ChevronLeft, LuChevronRight as ChevronRight, LuCheck as Check, LuClock as Clock } from 'react-icons/lu';

/**
 * Highly Reusable Sleek Dropdown Component
 * Features glassmorphic styling, custom icon support, status indicators (dotColor),
 * scrollable options, and automatic click-outside closing.
 */
export const SleekDropdown = ({
  icon: Icon,
  iconColor = "text-primary-500",
  label,
  headerTitle,
  options = [], // Array of { value, label, icon: OptionIcon, dotColor }
  value,
  onChange,
  widthClass = "w-44",
  ...
  return (
    <div className="relative inline-block" ref={dropdownRef}>
      ...
      >
        ...
        {renderTriggerLabel ? (
          ...
          <div className="flex items-center gap-2">
            {currentOption.dotColor && <span className={`w-2 h-2 rounded-full ${currentOption.dotColor}`} />}
      ...
      {isOpen && (
        <div className={`absolute left-0 mt-1.5 ${widthClass} bg-card border border-border rounded-xl shadow-lg z-50 py-1 ${maxHeightClass} overflow-y-auto animate-in fade-in zoom-in-95 duration-150 custom-scrollbar`}>
          {headerTitle && (
            <div className="px-3 py-1 text-[10px] font-extrabold text-muted-foreground uppercase tracking-wider border-b border-border/50 mb-1 sticky top-0 bg-card z-10">
              {headerTitle}
            </div>
          )}
          {options.map((option) => {
