...
import { LuPackage as Package, LuClock as Clock, LuCheck as CheckCircle2, LuCreditCard as CreditCard, LuX as XCircle, LuChevronRight as ChevronRight, LuChevronDown as ChevronDown, LuMapPin as MapPin } from 'react-icons/lu';
...
export default function OrderCard({ 
  ...
  let dateObj = parseDate(order.orderDate || order.createdAt);

  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-sm hover:border-primary-500/50 transition-colors group">
      {/* Header */}
      ...
      <div className="p-5 md:p-6 flex flex-col md:flex-row gap-6 justify-between items-start md:items-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <button 
            onClick={() => toggleOrderDetails && toggleOrderDetails(order.id)}
                  ...
                  <div key={idx} className="flex justify-between items-center bg-muted/20 p-4 rounded-xl border border-border">
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      {order.status === 'DELIVERED' && (() => {
                        ...
                        return (
                          <button
                            onClick={() => openReviewModal && openReviewModal(order.id, item.productId, item.productName, localReview)}
                            className="text-xs font-semibold text-primary-600 hover:text-primary-700 underline underline-offset-2 transition-colors"
                          >
                            {isReviewed ? 'Edit Rate' : 'Rate Product'}
                          </button>
                        );
                      })()}
                    </div>
